/*
 * === LiveBooster library Arduino example ===
 *
 ...
 *
 *    o Pins 40 (Serial1_TX),  39 (Serial1_RX) use for link to Heracles board < chipKIT uC32 >
 *    o Pins D1 (Serial1_TX),  D0 (Serial1_RX) use for link to Heracles board < ARDUINO M0 Pro >
 *    o Pins 18 (Serial1_TX),  19 (Serial1_RX) use for link to Heracles board < ARDUINO MEGA ADK >
 *    o Pins D1 (Serial1_TX),  D0 (Serial1_RX) use for link to Heracles board < LinkIt ONE >
 *
...
#define PRINTF DebugSerial.print
...
const char* appv_version = "Sample_Arduino_test V00.01";
...
int paramUdpdateCb (const LiveBooster_Param_t *ptrParam, const void *value, int val_len) {
...
void loop() {
	    ...
	    unsigned long timeToStopInMs;

	    /* 1 - Initializing LiveBooster Instance */
	    PRINTF ("\nLiveBooster Init:\n");
	    res = LiveBooster_Init(deviceId, apiKeyP1, apiKeyP2, &arduinoSerialImpl, &arduinoTimerImpl, &arduinoDebugImpl);

	    /* 2 -  Attach parameters, commands, resources, status, data*/
        if (res == OK) {
            PRINTF ("\nAttach parameters, commands, resources, status, data:\n");
        	res = LiveBooster_AttachCfgParameters  (setOfParam, SET_PARAM_NB, paramUdpdateCb);
        	res = LiveBooster_AttachCommands(setOfcommands, SET_COMMANDS_NB, mainCommandCb);
        ...
        if (res == OK) {
 	       ...
 	       /* Run one hour */
 	       timeToStopInMs = arduinoTimerImpl.millis() + 3600000;
 	       while (arduinoTimerImpl.millis() < timeToStopInMs) {
 	            if (res < 0) {
				...
				if ((Data.ToPublish == PUBLISH_DATA) &&
						( (arduinoTimerImpl.millis() - timeInMs) >= Data.PeriodInMs)) {
					timeInMs = arduinoTimerImpl.millis();
