 ...
 *
 *  - UDOO neo board :
 *    o Pins 31 (UART6_TX_DATA </dev/ttymxc5>),  30 (UART6_RX_DATA </dev/ttymxc5>) use for link to Heracles board.
 *    o Pin D4 set to 3.3V (reset of Heracles shield board).
 *
 *  - raspberry pi 3 board :
 *    o Pins 8 (UART0_TX_DATA [GPIO 14] </dev/ttAMA0>),  30 (UART0_RX_DATA [GPIO 15] </dev/ttAMA0>) use for link to Heracles board.
 *    o Pin D4 set to 3.3V (reset of Heracles shield board).
*/
...
#include "../LinuxImpl/LinuxSerialImpl.h"
#include "../LinuxImpl/LinuxTimerImpl.h"
#include "../LinuxImpl/LinuxDebugImpl.h"
...
const char* appv_version = "Sample_Linux_test V00.01";
...
int main(void)
{
    ...
    PRINTF ("\nLiveBooster Init:\n");
    res = LiveBooster_Init(deviceId, apiKeyP1, apiKeyP2, &linuxSerialImpl, &linuxTimerImpl, &linuxDebugImpl);
    ...
    /* 4 - Survey data from LoveObject platform */
    if (res == OK) {
    	   PRINTF ("\nStart LiveBooster cycle:\n");
		   /* Run one hour */
		   timeToStopInMs = linuxTimerImpl.millis() + 3600000;

		   while (linuxTimerImpl.millis() < timeToStopInMs) {
			 	 if (res < 0) {
				...
				if ((Data.ToPublish == PUBLISH_DATA) &&
						( (linuxTimerImpl.millis() - timeInMs) >= Data.PeriodInMs)) {
					timeInMs = linuxTimerImpl.millis();
